...
BrowserID.PageHelpers = (function() {
  ...
  function openPrimaryAuth(winchan, email, authURL, callback) {
    if(!(email && authURL)) {
      throw "cannot verify with primary without an email address and URL"
    }

    // Some BigTent IdPs need special sized windows.  If it is known what the
    // window size should be, open the window in that size now to reduce the
    // amount of flicker that occurs.
    var authWindowSize = proxyIdP.authWindowSize(authURL, email)
          || { w: 700, h: 375 };
    ...
    winchan.open({
      ...
      window_features: "width=" + authWindowSize.w + ",height=" + authWindowSize.h,
      params: helpers.toURL(authURL, {email: email})
