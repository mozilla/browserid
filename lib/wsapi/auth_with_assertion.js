...
exports.process = function(req, res) {
  ...
  logger.info('AOK PROXYIPD auth_with_assertion assertion=' + req.body.assertion);
  primary.verifyAssertion(req.body.assertion, function(err, email) {
    ...
    db.emailType(email, function(err, type) {
      ...
      if (type === 'primary' ||
          type === 'proxyidp') {
        // TODO aok - Is this right? or what should we do here...
      ...
      var preq = m.request({
        ...
        pres.on('end', function() {
          ...
          logger.info("successfully created primary or proxyidp acct for " + email + " (" + r.userid + ")");
          logger.info("AOK PROXYIDP dbwriter says=" + JSON.stringify(r));
