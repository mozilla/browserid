...
var MetricsFileTransport = function(options) {
  ...
  if (configuration.get('winston_logging_maxsize') > 0) {
    defaults.maxsize = configuration.get('winston_logging_maxsize');
    defaults.maxFiles = configuration.get('winston_logging_maxfiles');
  }
...
MetricsFileTransport.prototype.log = function(level, msg, meta, callback) {
  ...
  return File.prototype.log.call(this,
              level, JSON.stringify(entry), null, callback);
};
