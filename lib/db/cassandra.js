 ...
 *
 * TODO I'm skeptical about wtf the "connection pool" behavior in helenus
...
// URGENT TODO: replace helenus with this other client
...
exports.open = function(cfg, cb) {
  ...
  var options = {
    ...
    timeout: cfg.max_query_time || 5000,
  ...
  function doConnect() {
    ...
    client.connect(function(err) {
      ...
      client.cql("USE " + keyspace, function(err) {
  ...
  if (cfg.create_schema) {
    ...
    client.connect(function(err) {
      ...
      client.cql("CREATE KEYSPACE " + keyspace + " WITH replication = " + keyspaceProperties, function(err) {
        ...
        client.cql("USE " + keyspace, function(err) {
          ...
          function createNextTable(i) {
            if (i < schemas.length) {
              ...
              client.cql(schemas[i], function(err) {
...
exports.close = function(cb) {
  if (!client) { return cb() } // if it's already closed, quit silently? TODO
...
exports.emailInfo = function(email, cb) {
  client.cql(
    ...
    function(err, resp) {
      if (err) return cb(err);
      if (!resp || !resp[0]) { return cb(err, null) }
...
exports.stageUser = function(email, hash, cb) {
  secrets.generate(48, function(secret) {
    ...
    var query = "BEGIN BATCH" +
      ...
      " APPLY BATCH",
      args = [secret, true, false, email, hash, secret, email ];
    client.cql(query, args, 
      function(err) {
...
function addEmailToUser(userID, email, type, cb) {
  ...
  var query = "BEGIN BATCH" +
    ...
    " APPLY BATCH",
    args = [ userID, email, type, email, userID ];
  client.cql(query, args,
    function(err) {
      if (err) {
        logUnexpectedError(err);
    ...
    function(err, resp) {
      ...
      client.cql(
        ...
        function(er2, resp2) {
          ...
          client.cql(deleteQuery, deleteArgs,
            function(er) {
...
exports.createUserWithPrimaryEmail = function(email, cb) {
  ...
  var currentTime = uid = now(),
    query = "BEGIN BATCH" +
      " INSERT INTO user(id, lastPasswordReset, emails) VALUES(?, ?, ?)" +
      " INSERT INTO email(user, address, type) VALUES(?, ?, ?)" +
      " APPLY BATCH",
    args = [ uid, currentTime, [email], uid, email, 'primary' ];
  client.cql(query, args, 
    function(err) {
...
exports.stageEmail = function(existing_user, new_email, hash, cb) {
  secrets.generate(48, function(secret) {
    ...
    var query = "BEGIN BATCH" +
      ...
      " INSERT INTO staged_secret_to_email (secret, email) VALUES (?, ?)" +
      " APPLY BATCH",
      args = [ secret, false, existing_user, new_email, hash, secret, new_email ];
          ...
          client.cql(
            ...
            function(err, stagedSecrets) {
              ...
              var deleteQuery = "BEGIN BATCH" +
                ...
                " APPLY BATCH",
                deleteArgs = [ emailsToDelete, stagedEmails, secrets, uid, uid ];

              client.cql(deleteQuery, deleteArgs, cb);
            }
          );
        });  // ♫  o christmas tree ♫ 
      }
...
exports.createUnverifiedUser = function(email, hash, cb) {
  var uid = now(), // using Date for uuid
    ...
    " APPLY BATCH",
    args = [ uid, hash, uid, email, uid, email, false ];
...
exports.addTestUser = function(email, hash, cb) {
  var uid = now(),
    ...
    " APPLY BATCH",
    args = [ uid, hash, uid, email, uid, email ];
  client.cql(query, args,
    function(err) {
      if (err) logUnexpectedError(err);
