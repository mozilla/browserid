...
exports.open = function(cfg, cb) {
  ...
  // create the client
  function doConnect() {
    logger.debug("connecting to keyspace: " + keyspace);
    options.keyspace = keyspace;
    // in case we already initiated a connection to create schema,
    // don't make a second pool. TODO figure out if it hurts anything to
    // call h.ConnectionPool 2x.
    if (!client) client = new helenus.ConnectionPool(options);
    client.connect(function(err, keyspace) {
      logger.debug("connection to keyspace " + (err ? ("fails: " + err) : "established"));
      cb(err);
    });
  }
