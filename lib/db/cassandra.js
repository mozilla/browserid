...
function parseList(list, cb) {
  if (!list) return cb();
  // double-check list is of type Buffer
...
exports.updateIDPLastSeen  = function(domain, cb) {
  ...
  client.query("INSERT INTO idp (domain, lastSeen) VALUES(?, NOW())", [ domain ], cb);
}
...
exports.userOwnsEmail = function(uid, email, cb) {
  client.query(
    "SELECT address FROM email_user_to_address WHERE user = ?", [ uid ],
    function(err, resp) {
      // check if one of the rows has the email in it.
    }
  );
}
