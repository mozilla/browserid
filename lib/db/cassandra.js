 ...
 *
 * URGENT TODO: figure out best way to use cql3's `BATCH` statement to group
 *              multiple related inserts/updates/deletes. Also need to retry
 *              if some of a group of related statements fails. If BATCH isn't
 *              performant or stable yet, we'll need to make a meatier client.
 * 
    ...
    exports.emailToUID(o.email, function(err, uid) {
      ...
      client.cql(
        ...
        function(err, resp) {
          ...
          client.cql(
            "UPDATE email SET verified = false WHERE address IN ?", [ emails ],
            function(err) {
              if (err) return cb(err);
...
// TODO first do all the selects with high consistency value, then BATCH deletes?
