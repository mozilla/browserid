...
*/
function getValue(r, key) { return r && r.rows && r.rows[0] && r.rows[0].get(key.toLowerCase()) }
...
function getRowValue(row, key) { return row && row.get(key.toLowerCase()) }
...
function addEmailToUser(userID, email, type, cb) {
  ...
  var query = "BEGIN BATCH" +
    ...
    " APPLY BATCH",
    args = [ email, userID, userID, email, type];
  client.execute(query, args,
    function(err) {
      if (err) {
        logUnexpectedError(err);
...
exports.createUserWithPrimaryEmail = function(email, cb) {
  ...
  var lastPasswordReset = now(),
    ...
    query = "BEGIN BATCH" +
      ...
      " INSERT INTO email(user, address, type) VALUES(?, ?, ?)" +
      " APPLY BATCH",
    args = [ uid, lastPasswordReset, [email], uid, email, 'primary' ];
  client.execute(query, args, 
    function(err) {
