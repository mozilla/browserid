...
exports.updateIDPLastSeen  = function(domain, cb) {
  ...
  client.execute("INSERT INTO idp (domain, lastSeen) VALUES(?, NOW())", [ domain ], cb);
}
...
exports.stageUser = function(email, hash, cb) {
  secrets.generate(48, function(secret) {
    ...
    var query = "BEGIN BATCH" +
      ...
      " APPLY BATCH",
      args = [secret, true, null, email, hash, secret, email ];
...
exports.incAuthFailures = function(uid, cb) {
  ...
  client.execute(
    ...
    function(err, resp) {
      if (err) return cb(err);
      var incremented = getValue(resp, 'failedauthtries') + 1 || 1;
      client.execute(
