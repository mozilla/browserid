...
exports.open = function(cfg, cb) {
  ...
  if (cfg.create_schema) {
    ...
    createClient.connect(function(err) {
      if (err) {
        logUnexpectedError(err);
        return cb(err);
      }
      logger.debug("creating keyspace");
      createClient.cql("CREATE KEYSPACE " + keyspace + " WITH replication = " + keyspaceProperties, function(err) {
        ...
        createClient.cql("USE " + keyspace, function(err) {
          ...
          function createNextTable(i) {
            if (i < schemas.length) {
              ...
              createClient.cql(schemas[i], function(err) {
