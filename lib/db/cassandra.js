...
function addEmailToUser(userID, email, type, cb) {
  ...
  // TODO properly deleting from email_by_user is a bit complex. handle it.
  var query = "DELETE FROM email WHERE address = ?",
    args = [ email ],
    query2 = "INSERT INTO email(user, address, type) VALUES(?, ?, ?)",
    args2 = [ userID, email, type ],
    query3 = "INSERT INTO email_by_user(user, address) VALUES(?, ?)",
    args3 = [ userID, email ];
}
...
exports.userOwnsEmail = function(uid, email, cb) {
  var query = "SELECT COUNT(*) AS n FROM email_by_user WHERE address = ? AND user = ?",
    args = [ email, uid ];
}
...
exports.listEmails = function(uid, cb) {
  ...
  var query = "SELECT address FROM email_by_user WHERE user = ?",
    args = [ uid ];
}
