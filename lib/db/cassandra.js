...
exports.completeCreateUser = function(secret, cb) {
  // getAndDeleteRowForSecret
    // dbutils.withType
        ...
        var query = "INSERT INTO user(passwd, lastPasswordReset, failedAuthTries) VALUES(?,?)",
          args = [ o.passwd, now(), 0 ]
}
...
exports.createUserWithPrimaryEmail = function(email, cb) {
  ...
  var lastPasswordReset = now(),
    query = "INSERT INTO user(lastPasswordReset) VALUES(?)",
...
exports.userOwnsEmail = function(uid, email, cb) {
  var query = "SELECT COUNT(*) AS n FROM email_user_to_address WHERE address = ? AND user = ?",
    args = [ email, uid ];
}
...
exports.createUnverifiedUser = function(email, hash, cb) {
  var query = "INSERT INTO user(passwd, lastPasswordReset) VALUES(?, ?)",
    args = [ hash, now() ],
    // inside callback:
