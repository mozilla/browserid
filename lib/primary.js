...
function parseWellKnownBody(body, fromDomain, principalDomain, delegates, cb) {
  ...
  try {
    ...
    return process.nextTick(function() {
      if (cb) cb("malformed declaration of support for '" + fromDomain + "': " + e.toString());
    });
  }
...
var getWellKnown = function (fromDomain, principalDomain, delegates, cb) {
  ...
  req.on('error', function(e) {
    ...
    logger.debug(fromDomain + ' is not a browserid primary: ' + e.toString());
    handleProxyIDP();
  });
};
...
exports.checkSupport = function(principalDomain, cb, delegates) {
  ...
  getWellKnown(fromDomain, principalDomain, delegates,
    ...
    if (err) {
      ...
      try {
        var r = parseWellKnownBody(body, newDomain, principalDomain, cbdelegates, function (err, r) {
          if (err) {
            logger.debug(err);
            cb(err);
          } else {
        ...
        var msg = newDomain + ' is a broken browserid primary, malformed dec of support: ' + e.toString();
        logger.debug(msg);
        return cb(msg);
      }
    }
