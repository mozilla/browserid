...
var conf = module.exports = convict({
  env: {
    ...
    format: ["production", "local", "test_mysql", "test_json", "test_cassandra"],
    default: 'production',
    env: 'NODE_ENV'
  },
  bind_to: {
  ...
  /* TODO worth it to change MYSQL_USER to DB_USER? */
  database: {
    driver: {
      format: ['json', 'mysql', 'cassandra'],
      default: 'json'
    },
    user: {
      format: String,
    ...
    cassandra_keyspace_properties: {
      format: "string = \"{'class': 'SimpleStrategy', 'replication_factor': 1}\"",
      doc: "keyspace replication properties specified when creating a Cassandra keyspace"
    },
    cassandra_hosts: {
      format: 'array { string }* = ["localhost"]',
      doc: "list of cassandra hosts in the ring"
    },
    cassandra_cql_version: {
      format: 'string = "3.0.0"',
      doc: "version of cassandra CQL language used by client"
    }
  },
  smtp: {
    host: {
...
} else if (conf.get('env') === 'test_cassandra') {
  conf.set('database.driver', 'cassandra');
}
